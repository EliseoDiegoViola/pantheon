...
namespace GFSM {
    ...
    public class FiniteStateMachine<TInitialState> where TInitialState : class, IState, new() {
        ...
        public void DoTransition(string token) {
            ...
            {
                UnityEngine.Debug.Log(item.Token);
            }*/
            ...
            do {
                var transition = possible.Aggregate((t1,t2) =>
                {
                    int t1Dist = 0;
                    int t2Dist = 0;
                    var distanceTo1 = Stack.IndexOf(t1.To);
                    t1Dist = distanceTo1 == -1 ? t1.TransitionMode != Mode.Pop ? 0 : int.MaxValue : distanceTo1;
                    var distanceTo2 = Stack.IndexOf(t2.To);
                    t2Dist = distanceTo2 == -1 ? t2.TransitionMode != Mode.Pop ? 0 : int.MaxValue : distanceTo2;
